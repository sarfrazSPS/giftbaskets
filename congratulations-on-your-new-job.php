<?php include("includes/variables.php"); ?>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <?php include($root_path . "includes/header-links.php"); ?>

    <meta property="og:type" content="product" />

    <meta property="og:title" content="Congratulations on Your New Job " />

    <meta property="og:description" content="When you want to say congratulations on your new job, our PA Dutch gourmet foods and baked goods are loved and appreciated by everyone receiving them">
		  
    <meta property="og:url" content="https://padutchbaskets.com/congratulations-on-you-new-job.php"/>

    <meta property="og:site_name" content="PA Dutch Baskets" />
        <meta property="og:image" content="https://www.padutchbaskets.com/assets/images/products/savory-flavors/new-job.jpg" />

    <meta property="product:price:amount" content="119.50" />

    <meta property="product:price:currency" content="USD" />

    <meta property="og:availability" content="instock" />

    <title>"Congratulations on Your New Job | Lancaster PA</title>

    <meta name="description" content="When you want to say congratulations on your new job, our PA Dutch gourmet foods and baked goods are loved and appreciated by everyone receiving them" />

    <meta name="keywords" content="congratulations on your new job, congratulations gift baskets, congratulations graduate, congratulations on your new job, congratulations gift, congratulations gifts, lancaster pa"/>

    <link rel="canonical" href="https://padutchbaskets.com/congratulations-on-your-new-job.php">
</head>

<body>
    <div id="content" class="site-content">

    <header>
        <?php include($root_path . "includes/header-top-bar.php"); ?>
        <?php include($root_path . "includes/header-branding.php"); ?>
        <?php include($root_path . "includes/navbar.php"); ?>   
    </header>



<section id="" class="mt-5 mb-5">
    <div class="container-fluid">
        <div class="container">
            <div class="row">
                <div class="col-md-6">
                    <div class="row">
                        <div class="col-sm-2 small-images-outer">
                            <div class="small-images">
                            <!--4 side images -->
                                <img
                                class="img-responsive"
                                src="assets/images/products/savory-flavors/new-job.jpg"
                                alt="A great way to say congratulations on your new job with our well known PA Dutch gourmet foods that are loved by all and can be customized to the special occasion"
                                onmouseover="myFunction(this)"
                                />
                                            
                                <img
                                class="img-responsive"
                                src="assets/images/products/savory-flavors/pic2.jpg"
                                alt="Saying congratulations on your new job with our baked goods and gourmet foods from Lancaster PA that we customize for your occasion"
                                onmouseover="myFunction(this)"
                                />
                            
                              
                                
                            </div>
                        </div>
                        <div class="col-sm-10">
                            <div class="main-images">
                            <!--Main images -->
                                
                                   <img
                                class="img-responsive"
                                src="assets/images/products/savory-flavors/new-job.jpg"  alt="Our congratulations gift baskets are customized to any occasion and offer delicious PA Dutch gourmet foods chocolates and more"  id="image_box" class="img-responsive"  />
                                
                            <!--Main images -->
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                    <p class="prod-id">Product ID: 041</p>
                    <h3 class="prod-title">"Savory Flavors" Congratulations on Your New Job </h3>
                      <span class="price">
                        <span class="prod-price" id='price_per_item' >$119.50</span>
                            <span class="prod-stars">
                                <svg class="es-star" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9.88552 5.85364H0L4.94427 9.72919L8 12.1244L12.9443 16L11.0557 9.72919L16 5.85364H9.88552Z" fill="#7a6e58"></path><path fill-rule="evenodd" clip-rule="evenodd" d="M6.53216 5.2863L8.13765 0L9.74314 5.2863H6.53216ZM4.92025 10.5937L7.5658 12.6674L3.27832 16L4.92025 10.5937Z" fill="#7a6e58"></path></svg><svg class="es-star" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9.88552 5.85364H0L4.94427 9.72919L8 12.1244L12.9443 16L11.0557 9.72919L16 5.85364H9.88552Z" fill="#7a6e58"></path><path fill-rule="evenodd" clip-rule="evenodd" d="M6.53216 5.2863L8.13765 0L9.74314 5.2863H6.53216ZM4.92025 10.5937L7.5658 12.6674L3.27832 16L4.92025 10.5937Z" fill="#7a6e58"></path></svg><svg class="es-star" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9.88552 5.85364H0L4.94427 9.72919L8 12.1244L12.9443 16L11.0557 9.72919L16 5.85364H9.88552Z" fill="#7a6e58"></path><path fill-rule="evenodd" clip-rule="evenodd" d="M6.53216 5.2863L8.13765 0L9.74314 5.2863H6.53216ZM4.92025 10.5937L7.5658 12.6674L3.27832 16L4.92025 10.5937Z" fill="#7a6e58"></path></svg><svg class="es-star" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9.88552 5.85364H0L4.94427 9.72919L8 12.1244L12.9443 16L11.0557 9.72919L16 5.85364H9.88552Z" fill="#7a6e58"></path><path fill-rule="evenodd" clip-rule="evenodd" d="M6.53216 5.2863L8.13765 0L9.74314 5.2863H6.53216ZM4.92025 10.5937L7.5658 12.6674L3.27832 16L4.92025 10.5937Z" fill="#7a6e58"></path></svg><svg class="es-star" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9.88552 5.85364H0L4.94427 9.72919L8 12.1244L12.9443 16L11.0557 9.72919L16 5.85364H9.88552Z" fill="#7a6e58"></path><path fill-rule="evenodd" clip-rule="evenodd" d="M6.53216 5.2863L8.13765 0L9.74314 5.2863H6.53216ZM4.92025 10.5937L7.5658 12.6674L3.27832 16L4.92025 10.5937Z" fill="#7a6e58"></path></svg>
                            </span>
                            <span class="prod-seperator"> | </span>
                            <span class="prod-rev-nos">37 Reviews</span>
                    </span>
                  
                    <p class="prod-copy">Make Your Selections</p>
                    <form class="prod-form" target="paypal" action="" method="post">
                        <table id="main_tbl">

                        <tr>
							
							<td>
                                
                                    <input type="hidden" name="on0" value="Choose Your Pie">
                                    <div class="form-label">Choose Your Pie</div>
                                    <select name="os0" id="choosePie">
                                        <option value="Apple">Apple </option>
                                        <option value="Banana">Banana </option>
                                      
                              </select>
                                
                            </td>
							 
							<td>
                                
                                    <input type="hidden" name="on0" value="Choose Your Fudge">
                                    <div class="form-label">Choose Your Fudge</div>
                                    <select name="os0" id="chooseFudge">
                                        <option value="Chocolate">Chocolate </option>
                                        <option value="Choc/Peanut Butter">Choc/Peanut Butter </option>
                                      
                              </select>
                                
                            </td>
							
                        </tr>

                        <tr>
                            <td>
                                <input type="hidden" name="on2" value="Card Message">
                                <div class="form-label form-label-margin">Card Message</div>
                                <input type="text" class="form-input-text" id="cardMessage" name="os2" maxlength="200" oninput="checkCharacterCount(this.value)"> <span id="message" style="display: none; font-size: medium; color:red;"></span> 
                            </td>
                            
                        </tr>

                        </table>

                        <div>
                                    <h5 class="customization-head">Choose Your Customization Below </h5>
                                    <div class="services">
                                                                        
                                        <div class="custom-options-services">
    
                                        <div class="row justify-content-center">
    
                                                 <div class="col-sm-new-2 mb-new">
    
                                                    <label for="tag3">
    
                                                        <input type="checkbox" id="tag" class="" value="congrats1">
    
                                                        
                                                        
                                                      <img src="assets/images/tags/congrats.png" alt="Sending our PA Dutch baskets is a great way to say congratulations on your new job with our baked goods loved by everyone  " class="circle">
    
                                                    </label>
    
                                                </div>
											
                                                <div class="col-sm-new-2 mb-new">
    
                                                    <label for="tag3">
    
                                                        <input type="checkbox" id="tag" class="" value="congrats2">
    
                                                      <img src="assets/images/tags/congrats1.png" alt="Our congratulations gift offers the most delicious baked goods and gourmet foods from Lancaster PA and customized to the occasion" class="circle ">
    
                                                    </label>
    
                                                </div>
    
                                                <div class="col-sm-new-2 mb-new">
    
                                                    <label for="custom-front">
    
                                                        <input type="checkbox" id="custom-front" class="" value="Ann3">
    
                                                       <img src="assets/images/tags/congrats2.png" alt="Looking to say congratulations on your new job, our gift baskets are the perfect gift loved by all" class="circle ">
    
                                                    </label>
    
                                                </div>
    
    
                                                <div class="col-sm-new-2 mb-new">
    
                                                    <label for="Tag">
    
                                                        <input type="checkbox" id="Tag" class="" value="Ann4">
    
                                                        <img src="assets/images/tags/congrats3.png" alt="A very special gift to say congratulations on your new job with delicious gourmet foods " class="circle ">
    
                                                    </label>
    
                                                </div>
                                                
    
                                                <div class="col-sm-new-2 mb-new">
    
                                                    <label for="tag">
    
                                                        <input type="checkbox" id="tag" class="" value="Ann5">
    
                                                      <img src="assets/images/tags/congrats4.png" alt="Say congratulations with our special PA Dutch gift baskets that we can deliver and customize to any occasion" class="circle ">
    
                                                    </label>
    
                                                </div>
											
											  <div class="col-sm-new-2 mb-new">
    
                                                    <label for="tag">
    
                                                        <input type="checkbox" id="tag" class="" value="Ann6">
    
                                                       <img src="assets/images/tags/congrats5.png" alt="Want a gift that they will love while sayhing congratulations on your new job, try our PA Dutch gourmet foods and baked goods loved by everyone" class="circle ">
    u
                                                    </label>
    
                                                </div>
											
											
                                     <div class="col-sm-new-2 mb-new">
    
                                                    <label for="tag">
    
                                                        <input type="checkbox" id="tag" class="" value="ThinkingYou">
    
                                                       <img src="assets/images/tags/thinking-of-you.png" alt="A delicious way to say congratulations with baked goods from Lancaster PA well known for the best gourmet foods found anywhere" class="circle ">
    
                                                    </label>
    
                                                </div>
    
                                               
    
    
                                                <div class="col-sm-new-2 mb-new">
    
                                                    <label for="tag">
    
                                                        <input type="checkbox" id="tag" class="" value="Get Well">
    
                                                       <img src="assets/images/tags/no-custom.png"  alt="Our Amish gourmet foods are a great way to say congratulations on your new job" class="circle ">
    
                                                    </label>
    
                                                </div>
    
    
                                                <div class="col-sm-4 col-sm-new-4 text-center">
                                                    
                                                        <b>
                                                            Can't find what you want? <br>
                                                            Give us a call! 717-368-3973
    
                                                        </b>
                                                        
                                                </div>
                                        </div>
                                        </div>       
                            <!--NEW::Code End Here-->
                            </div>
                        </div>

                        <div class="form-qty-btn">
                            <div class="form-qty-left">
                                <span class="prod-form-qty">QTY </span> <input id="productQty" class="form-qty-input" type="number" step="1" min="1" max="1000" value="1" name="qty" >
                            </div>
                            <div class="form-qty-right">
                                <button type="submit" id="addToCartBtn">Add to Cart</button>
                            </div>
                        </div>

                    </form>
                </div>
                <div class="col-sm-12 mt-5">
                    <h2 class="cgb-head">Content for your Gift Basket</h2>
                    <div class="cgb-copy">
                        <div class="cgb-copy-left">
							
						
                             <p>* 2 Gourmet Cheeses
							<p>* Gourmet Mustard</p>
                            <p>* Apple or Shoo Fly Pie (Your Choice)</p>
							<p>* Delicious Blueberry Bread (Your Choice of Flavor)</p>
							
							<p>* Fudge (Your Choice of Flavor)</p>
							<P>* Peanut Butter Brittle</P>
							<p>* Chocolate w/Vanilla Whoopie Pie</p>
							
							
                        </div>
                        <div class="cgb-copy-right">
						
							<p>* Peanut Butter Balls</p>
							<p>* Cinnamon Stick (Filled w/Cream Cheese) </p>
							
							<p>* Emma's Popcorn</p>
							
							<p>* Lancaster Party Mix</p>
							<p>* Chocolate Pretzel Rods</p>
							<p>* Martins Pretzels</p>

							
							<p>* Customization</p>
							
                        </div>
                    </div>

                    <h2 class="cgb-subhead">Congrats on Your New Job</h2>
                    <p class="cgb-subcopy">When looking for that very sepecial way to say congratulations on your new job, our PA Dutch gourmet foods, baked goods, jams, chocoaltes and so much more are getting rave reviews.</P><p class="cgb-subcopy"> We are happy to offer these delicious foods from the well known Dutch in Lancaster PA. We also customize to any occasion. If you can't find what you want, give us a call.</p>




                </div>
            </div>
        </div>
    </div>
</section>
 <!-- do not remove this div, it will be using to pass data -->
 <div class="product-metadata" 
    data-product-id="product-041"
    data-product-name="Congratulations on Your New Job"
    data-product-price-regular="119.50"
    data-product-shipping-price="0"
    data-product-flavor-pie="yes"
    data-product-flavor-bread="yes"
    data-product-promo=""
    data-product-card-msg=""
    data-product-customization=""
    data-product-qty=""
    data-product-cart_img="new-job.jpg" >
</div>
<!-- image to be display on cart page must be in root images folder   -->


<?php
include("includes/footer.php");
?>
<script>
    function myFunction(smallImg){var fullImg=document.getElementById("image_box");fullImg.src=smallImg.src}
</script>
<script> var product_details ="Congratulations Graduate - Basket of Treats <br/>"; </script>
<script src="assets/js/paypal.js"></script>

<script type="text/javascript">
    function toggleMenu() {
    var x = document.getElementById("menu");
    if (x.className === "topnav") {
    x.className += " visible";
    console.log("open");
    }
    else {
    x.className ="topnav";
    console.log("closed");
    }
    }
</script>